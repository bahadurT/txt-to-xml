...
# example: person 75 133 76 147
#          person 279 64 66 168
# -------------------------------
...
dir_="Annotations-10-05-2021\\"
IMG_PATH = dir_+"images"
fw = os.listdir(IMG_PATH)
# path of save xml file
save_path = dir_+'xml' # keep it blank

# txt_folder is txt file root that using darknet rectbox
txt_folder = dir_+'labels'
...
def extract_coor(txt_file, img_width, img_height):
    ...
    x_min_rect = x_rect_mid
    x_max_rect = x_rect_mid + width_rect
    y_min_rect = y_rect_mid
    y_max_rect = y_rect_mid +height_rect

    print(x_min_rect, x_max_rect, y_min_rect, y_max_rect)
...
for line in fw:
    ...
    ff = open('xml\\%s.xml' % (img_name[:-4]), 'w')
